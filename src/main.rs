    ...
    loop {
        ...
        match event::read()? {
            Event::Key(key) => match key.code {
                KeyCode::Char('q')
                    if !app.searching() && !app.confirming_delete() && !app.renaming() =>
                {
                    break;
                }
                ...
                KeyCode::Backspace => app.key('\u{8}'),
                KeyCode::Esc => app.key('\u{1b}'),
                KeyCode::Char(character) => app.key(character),
                _ => {}
            },
...
fn execute_action(
    ...
    match action {
        UiAction::Trash(index) => {
            trash::delete(&app.sessions[index].path)?;
            app.sessions.remove(index);
            app.selected = 0;
        }
        UiAction::Rename(index, title) if !title.is_empty() => {
            rename_session(&app.sessions[index], &title)?;
            app.sessions[index].title = title;
        }
        UiAction::Rename(_, _) => {}
        UiAction::Launch(index, action) => {
            let Some(command) = launch_command(&app.sessions[index], action) else {
                return Ok(());
            };
            disable_raw_mode()?;
            execute!(
